...
#Row 1 widgets
from_label = ttk.Label(window,text="From: ", font=main_font)
from_label.grid(row=1,column=0, padx=10,pady=10,sticky="NW")

to_label = ttk.Label(window, text="To:", font=main_font)
...
from_entry.bind("<Any-KeyPress>", convert)

to_entry = ttk.Entry(window, font=main_font, justify="right", textvariable=default2)
to_entry.grid(row=1, column=1)

#Row 3 widgets
from_selection = ttk.Combobox(window, font=main_font,values=unit_types[set_type.get()])
from_selection.grid(row=2,column=0, sticky="N")
from_selection.current(0)
from_selection.bind("<<ComboboxSelected>>", convert)


to_selection = ttk.Combobox(window, font=main_font, values=unit_types[set_type.get()])
